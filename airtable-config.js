// 🛡️ Configuración CORREGIDA de Airtable API - Fix Solicitudes de Acceso
// airtable-config.js - Versión con corrección completa para solicitudes de acceso

console.log('🚀 Cargando airtable-config.js (VERSIÓN CON FIX SOLICITUDES ACCESO)...');
...
class AirtableAPI {
    constructor() {
        console.log('🔧 Inicializando AirtableAPI con fix para solicitudes de acceso...');
        ...
        setTimeout(async () => {
            try {
                ...
                if (isConnected) {
                    ...
                    // NUEVO: Verificar tabla de solicitudes de acceso
                    await this.verifyAccessRequestsTable();
                } else {
    ...
    // NUEVA FUNCIÓN: Verificar tabla de solicitudes de acceso
    async verifyAccessRequestsTable() {
        console.log('🔍 Verificando tabla de solicitudes de acceso...');
        
        try {
            const result = await this.makeRequest(`${this.tables.solicitudesAcceso}?maxRecords=1`);
            
            if (result.records !== undefined) {
                console.log('✅ Tabla SolicitudesAcceso verificada correctamente');
                console.log(`📊 Registros en tabla: ${result.records.length}`);
                
                if (result.records.length > 0) {
                    console.log('📋 Campos detectados:', Object.keys(result.records[0].fields));
                }
                
                return true;
            }
        } catch (error) {
            console.error('❌ Error verificando tabla SolicitudesAcceso:', error);
            
            // Intentar con nombres alternativos
            const alternativeNames = ['Solicitudes_Acceso', 'Solicitudes Acceso', 'AccesosPortal'];
            
            for (const altName of alternativeNames) {
                try {
                    console.log(`🔄 Intentando con nombre alternativo: ${altName}`);
                    const altResult = await this.makeRequest(`${altName}?maxRecords=1`);
                    
                    if (altResult.records !== undefined) {
                        console.log(`✅ Tabla encontrada con nombre: ${altName}`);
                        this.tables.solicitudesAcceso = altName;
                        return true;
                    }
                } catch (altError) {
                    console.log(`❌ No encontrada como: ${altName}`);
                }
            }
            
            console.error('⚠️ No se pudo verificar la tabla de solicitudes de acceso');
            return false;
        }
    }
    ...
    // MÉTODO CORREGIDO: getSolicitudesAcceso con mejor manejo de errores
    async getSolicitudesAcceso() {
        console.log('📋 Obteniendo solicitudes de acceso...');
        
        try {
            const result = await this.makeRequest(this.tables.solicitudesAcceso);
            
            console.log(`✅ Solicitudes de acceso obtenidas: ${result.records ? result.records.length : 0}`);
            ...
            if (solicitudes.length > 0) {
                console.log('📋 Muestra de solicitud:', {
                    id: solicitudes[0].id,
                    email: solicitudes[0].email,
            ...
            if (error.message.includes('404') || error.message.includes('NOT_FOUND')) {
                console.error('⚠️ La tabla SolicitudesAcceso no se encuentra. Verificar nombre en Airtable.');
                
                // Guardar en localStorage como señal para el portal
                localStorage.setItem('hospital_access_table_error', 'true');
            }
    ...
    async createSolicitudAcceso(solicitudData) {
        ...
        try {
            ...
            const localRequests = JSON.parse(localStorage.getItem('hospital_access_requests') || '[]');
            localRequests.push({
                ...safeData,
                id: result.id,
                fechaCreacion: new Date().toISOString()
            });
            localStorage.setItem('hospital_access_requests', JSON.stringify(localRequests));
            ...
            // Si es error de tabla no encontrada, guardar localmente
            if (error.message.includes('404') || error.message.includes('NOT_FOUND')) {
                console.warn('⚠️ Tabla no encontrada, guardando localmente...');
                ...
                return { id: localRequest.id, fields: localRequest };
            }
    ...
    async retryCreateSolicitudAcceso(originalData) {
        ...
        const data = {
            fields: {
                ...
                fechaSolicitud: originalData.fechaSolicitud || new Date().toISOString(),
    ...
    // NUEVO MÉTODO: Test específico para solicitudes de acceso
    async testAccessRequestsTable() {
        ...
        try {
            ...
            return {
                success: true,
                tableName: this.tables.solicitudesAcceso,
                records: result.records ? result.records.length : 0,
                fields: result.records && result.records.length > 0 ? Object.keys(result.records[0].fields) : [],
                sampleData: result.records ? result.records[0] : null
            };
    ...
    getStatus() {
        return {
            ...
            tables: this.tables,
            timestamp: new Date().toISOString(),
            version: '5.3-fix-access-requests',
            features: [
                'CORREGIDO: getSolicitudesAcceso con mejor manejo de errores',
                'CORREGIDO: createSolicitudAcceso con fallback local',
                'CORREGIDO: Verificación de tabla SolicitudesAcceso al iniciar',
                ...
                'NUEVO: Método testAccessRequestsTable para debugging',
            ...
            accessRequestsStatus: {
                'Table verification': 'Automatic on initialization',
                'Error handling': 'Fallback to localStorage if table not found',
                'Debug method': 'testAccessRequestsTable() available',
                'Alternative names': ['SolicitudesAcceso', 'Solicitudes_Acceso', 'Solicitudes Acceso', 'AccesosPortal']
            }
        };
    }
}
...
try {
    console.log('🔧 Creando instancia global con fix para solicitudes de acceso...');
    window.airtableAPI = new AirtableAPI();
    console.log('✅ window.airtableAPI creado exitosamente (versión con fix solicitudes de acceso)');
} catch (error) {
    ...
    window.addEventListener('airtableConnectionUpdate', function(event) {
        ...
        if (typeof updateConnectionStatus === 'function') {
            ...
            const message = event.detail.connected 
                ? '✅ Conectado (fix solicitudes acceso activo)' 
                : 'Modo Local (fix solicitudes acceso activo)';
...
try {
    window.debugAirtableConnection = function() {
        ...
        console.log('🔐 Estado Solicitudes Acceso:', status.accessRequestsStatus);
...
// NUEVA FUNCIÓN: Debug específico para solicitudes de acceso
window.debugAccessRequests = async function() {
    console.log('🔍 DEBUG DE SOLICITUDES DE ACCESO');
    console.log('===============================================');
    ...
    try {
        // Test de tabla
        const tableTest = await window.airtableAPI.testAccessRequestsTable();
        console.log('📋 Test de tabla:', tableTest);
        ...
        if (solicitudes.length > 0) {
            console.log('📋 Muestra de solicitud:', solicitudes[0]);
        }
        
        // Verificar datos locales
        const localData = localStorage.getItem('hospital_access_requests');
        const localRequests = localData ? JSON.parse(localData) : [];
        console.log('💾 Solicitudes locales:', localRequests.length);
        
        return {
            tableTest,
            cloudRequests: solicitudes.length,
            localRequests: localRequests.length,
            tableError: localStorage.getItem('hospital_access_table_error') === 'true'
        };
...
console.log('✅ airtable-config.js VERSIÓN CON FIX SOLICITUDES ACCESO cargado');
console.log('🔐 Corrección específica para solicitudes de acceso implementada');
...
console.log('🛠️ Para diagnóstico general: debugAirtableConnection()');
console.log('🛠️ Para diagnóstico de accesos: debugAccessRequests()');
...
setTimeout(async () => {
    if (window.airtableAPI && typeof window.debugAirtableConnection === 'function') {
        ...
        // Verificar tabla de solicitudes de acceso
        const hasAccessTable = window.airtableAPI.tables.solicitudesAcceso;
        console.log(`🔐 Tabla solicitudes acceso configurada: ${hasAccessTable}`);
        
        // Test rápido de conexión a solicitudes de acceso
        try {
            const testResult = await window.airtableAPI.testAccessRequestsTable();
            if (testResult.success) {
                console.log('✅ Tabla SolicitudesAcceso verificada correctamente');
                console.log(`📋 Campos detectados: ${testResult.fields.join(', ')}`);
            } else {
                console.warn('⚠️ No se pudo verificar tabla SolicitudesAcceso:', testResult.error);
            }
        } catch (error) {
            console.error('❌ Error verificando tabla:', error);
        }
        
        console.log('\n🎉 SISTEMA COMPLETAMENTE OPERATIVO CON FIX PARA SOLICITUDES DE ACCESO');
    }
}, 3000);