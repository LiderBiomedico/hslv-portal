...
// NUEVA FUNCIÓN: Normalizar estado a mayúsculas
...
// MÉTODO CORREGIDO: getSolicitudesAcceso con normalización de estados
async getSolicitudesAcceso() {
    ...
    try {
        ...
        const solicitudes = result.records.map(record => {
            ...
            if (fields.estado) {
                fields.estado = this.normalizeEstado(fields.estado);
            }
            
            return {
                id: record.id, // Este es el ID real de Airtable
                ...fields // Los campos con estado normalizado
            };
        });
        
        // Log detallado para debugging
        if (solicitudes.length > 0) {
            console.log('📋 Muestra de solicitud (normalizada):', {
                id: solicitudes[0].id,
...
getStatus() {
    return {
        ...
        tables: this.tables,
        timestamp: new Date().toISOString(),
        version: '5.4-fix-estados-solicitudes',
        features: [
            'NUEVO FIX: Normalización de estados a mayúsculas',
            'CORREGIDO: Estados "Pendiente" → "PENDIENTE" automáticamente',
            'CORREGIDO: getSolicitudesAcceso normaliza estados',
            'CORREGIDO: Filtros funcionan con cualquier variación de estado',
            'CORREGIDO: Campo solicitudId removido para evitar error 422',
            'CORREGIDO: Área biomédica funciona perfectamente',
            ...
            'NUEVO: Método normalizeEstado para consistencia',
        ...
        estadoNormalization: {
            'Description': 'Estados normalizados automáticamente a mayúsculas',
            'Examples': [
                'Pendiente → PENDIENTE',
                'pendiente → PENDIENTE',
                'PENDIENTE → PENDIENTE',
            ...
            'Method': 'normalizeEstado()',
            'Applied': 'getSolicitudesAcceso() y filtros del portal'
        },
        biomedCorrections: {
        ...
        accessRequestsStatus: {
            ...
            'Estado normalization': 'All states normalized to uppercase'
        }
    };
}